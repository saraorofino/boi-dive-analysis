...
There is a different map for each of the three site thresholds (i.e. 1, 3, or 5 dives)  
...
dive_sites <- compare_sites_filtered %>% 
  filter(time_thresh == 1 & site_threshold == 1 & dive_site == 1) %>% 
...
For the final site list, remove any dive sites that overlap with known anchorages (29 sites total) and add any sites from the Spotting Giant Seabass dataset. Below is a map of the final dive site locations.  
<br>   
```{r site-harbor-overlap, include=F}
# Unique dive sites 
unique_sites <- dive_sites %>% 
  mutate(site_type = 'AIS') #3200 sites

harbor_overlap <- unique_sites %>% 
  left_join(island_anchorages_clean %>% 
              dplyr::select(lat_bin, lon_bin, anchorage_gfw_label, anchorage_island, harbor)) %>% # Only 29 sites 
  mutate(site_type = ifelse(!is.na(harbor), 'Harbor', site_type))
...
ci_sites_binned <- ci_sites %>% 
  ...
  mutate(site_type = "SGSB") %>% 
  dplyr::select(lat_bin, lon_bin, site_type)

# Remove sites that are harbors
# Add in sites from SGSB
dive_sites_rm_harbor <- harbor_overlap %>% 
  filter(site_type != "Harbor") %>% 
  bind_rows(ci_sites_binned) %>% 
  group_by(lat_bin, lon_bin) %>% 
  mutate(n=n()) %>% 
  ungroup() %>% 
  # Remove SGSB sites that are also identified in AIS
  mutate(repeated = ifelse(site_type == 'SGSB' & n==2, 'yes', 'no')) %>% 
  filter(repeated == 'no') %>% 
  # Label sites as both if they were overlapping
  mutate(site_type = ifelse(n==2, "Both AIS & SGSB", site_type),
         site_id = 1:nrow(.)) %>% 
  dplyr::select(site_id, lat_bin, lon_bin, site_type) %>% 
  mutate(site_type = factor(site_type, levels = c("AIS", "SGSB", "Both AIS & SGSB")))
```
...
final_site_map <- ggplot() + 
  geom_sf(data = ca_counties, fill = NA) + 
  geom_sf(data = ci_shp, fill = NA) + 
  geom_point(data = dive_sites_rm_harbor,
             aes(x=lon_bin,
                 y=lat_bin,
                 color = site_type),
             alpha = 0.7,
  ...
  scale_x_continuous(expand = c(0,0)) + 
  scale_y_continuous(expand = c(0,0)) + 
  scale_color_manual(breaks = c("AIS", "SGSB", "Both AIS & SGSB"),
                     values = c('cadetblue', 'firebrick4', 'darkslateblue')) + 
  labs(x="",
       y="",
       color="Site Type") + 
  theme_bw() +
...
# Save
write_csv(dive_sites_rm_harbor, file.path(project_data_path, "processed", "dive_sites_1hr_1vessel.csv"))
```
