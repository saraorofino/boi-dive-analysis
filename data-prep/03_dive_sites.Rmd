...
Some of these anchorages appear likely associated with the Channel Islands. We narrow down these results for just the anchorages being shown near the islands by including those with the following labels: Anacapa Isl, Avalon, AZ A, BF, Btai, Catalina Is, Isthmus Cove, Prisoners, San Diego, San Miguel Is, Santa Barbara Island, Santa Catalina Is, Santa Cruz Island, Santa Rosa Is, SantaBarbara, Scorpion Anch, Whites Cove Catalina, and Wilson Cove  
...
island_anchorages_clean <- island_anchorages %>% 
  ...
  dplyr::select(lat, lon, anchorage_id = s2id, anchorage_gfw_label = label, anchorage_island,
                ...
                unique_active_ssvid, unique_total_ssvid, lat_bin, lon_bin)
...
```
```{r check, include=F}
# Positions over 12 hours only
over_12hr <- binned_locations %>% 
  filter(total_hours >= 12) # All 15 vessels at 250 locations... 

# See if/how many of these positions overlap with harbors
harbor_overlap <- over_12hr %>% 
  left_join(island_anchorages_clean, by = c("lat_bin", "lon_bin")) %>% 
  filter(!is.na(anchorage_id)) %>% 
  dplyr::select(lat_bin, lon_bin, anchorage_id, anchorage_gfw_label, anchorage_island) %>% 
  distinct() # Only 7... 
```

<br>  

It looks like only 7 of the locations are the same grid cells identified as anchorages in the GFW anchorage dataset. We can double check after the final "dive site" determination to ensure that none of the dive sites overlap with harbors and ports.    
...
There is a different map for each of the site thresholds (i.e. 10 dives per year, 20 dives per year, etc.)

```{r map-dive-sites}
# Identify thecoastal sites that aren't in the CI 
...
```{r compare-dive-sites, include=F}
