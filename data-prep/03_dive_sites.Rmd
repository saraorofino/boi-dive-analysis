...
This final site list: 

  - Remove any dive sites that overlap with known anchorages (29 sites total). 
  - Includes only sites for the four Northern Channel Islands (San Miguel, Santa Rosa, Santa Cruz, and Anacapa)  
  - Includes only dive sites xx meters from shore    
<br>   

```{r site-harbor-overlap, include=F}
# Unique dive sites 
unique_sites <- dive_sites_island %>% 
  mutate(site_type = 'AIS') #2464 sites
...
# Keep only Northern CI
# Remove sites > 10000 meters (10 km)
dive_sites_final <- harbor_overlap %>% 
  ...
  # Fix numbering 
  mutate(site_id = 1:nrow(.)) %>% 
  dplyr::select(site_id, lat_bin, lon_bin, avg_distance_shore_m, nearest_island) #1428 sites
...
final_site_map <- ggplot() + 
  ...
  geom_point(data = dive_sites_final,
             aes(x=lon_bin,
                 y=lat_bin,
                 color = nearest_island),
             alpha = 0.7,
...
# Save
write_csv(dive_sites_final, file.path(project_data_path, "processed", "dive_sites_1hr30in_1vessel_northern_ci_labeled.csv"))
```
