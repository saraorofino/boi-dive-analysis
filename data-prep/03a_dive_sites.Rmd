...
We use AIS data from 2016 - November 30, 2022 gridded to 0.001 x 0.001 degrees and retrieve all positions where vessels are moving slower than 1 knot, which we consider positions where the vessel is stationary.      
...
At this stage we only remove sites that overlap with known anchorages (29) and those that are further than 10 km from shore. These sites will be further subset for analysis.  
...
unique_sites <- dive_sites_island %>% 
  mutate(site_type = 'AIS') #2673 sites
...
dive_sites_final <- harbor_overlap %>%
  ...
  # Fix numbering 
  mutate(site_id = 1:nrow(.)) %>% 
  dplyr::select(site_id, lat_bin, lon_bin, avg_distance_shore_m, nearest_island, island_group) #2605 sites

## Check sites in Northern CI only
check_north_ci <- dive_sites_final %>% 
  filter(island_group == "Northern") #1526 sites
```
