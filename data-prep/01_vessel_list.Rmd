...
Searched the potential matches on Marine Traffic to identify which ones are likely our dive vessels based on vessel class and area of operation. This list includes 14 unique vessel names. The following vessels are found in Marine Traffic with a correct MMSI:  
...
Pull tracks for October 2020 for the 14 matched vessels and check to see if this lines up with activity in the Channel Islands
...
ssvids <- c('366813530', '367036830','368926530') 
...
Still need to verify which ssvid is correct for Islander, and see if the Ranger ssvid seems right 
```{r sql-vessel-list}
