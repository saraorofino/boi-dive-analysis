...
```{r data-prep-heatmaps, warning=F, message=F, echo=F}
# Northern sites by island
sites_by_island <- dive_sites %>% 
  filter(island_group == 'Northern') %>% 
  group_by(nearest_island) %>% 
  summarize(n_sites = n()) %>% 
  ungroup()

# Filter for just one set of dives
scenario1_unique <- scenario1 %>% 
  filter(mpa_definition == "Marine Reserve") %>% 
  group_by(year, month, site_id, nearest_island, lat_bin, lon_bin) %>% 
  summarize(n_dives_at_site = n()) %>% 
  ungroup() %>% 
  group_by(year, month, nearest_island) %>% 
  summarize(n_dives_total = sum(n_dives_at_site)) %>% 
  ungroup() %>% 
  left_join(sites_by_island, by = "nearest_island") %>% 
  mutate(nearest_island = factor(nearest_island, levels = c("San Miguel", "Santa Rosa", "Santa Cruz", "Anacapa")),
         month = as.factor(month),
         dive_ratio = n_dives_total / n_sites)

scenario2_unique <- scenario2 %>% 
  filter(mpa_definition == "Marine Reserve") %>% 
  group_by(site_id, lat_bin, lon_bin) %>% 
  summarize(n_dives = n()) %>% 
  ungroup()

scenario3_unique <- scenario3 %>% 
  filter(mpa_definition == "Marine Reserve") %>% 
  group_by(site_id, lat_bin, lon_bin) %>% 
  summarize(n_dives = n()) %>% 
  ungroup()

# Number of dives & dive sites by island
scen1_summary <- scenario1_unique %>% 
  group_by(nearest_island, n_dives) %>% 
  summarize(n_dive_sites = n()) %>% 
  ungroup()
  
```

```{r heatmaps, message=F, warning=F, echo=F}
scen1_heatmap <- ggplot() + 
  geom_tile(data=scenario1_unique, aes(y=year, x=month, fill=dive_ratio)) + 
  scale_y_continuous(expand = c(0,0),
                     breaks = seq(2016,2022, by=1)) + 
  scale_x_discrete(expand = c(0,0),
                   labels = c("Jan", "Feb", "Mar", "Apr", "May", "Jun",
                              "Jul", "Aug", "Sept", "Oct", "Nov", "Dec")) + 
  scale_fill_continuous_sequential(palette = "Heat",
                                   trans = "log") +
  # scale_fill_continuous_divergingx(palette = 'RdYlBu', mid = 1, p1=0.2, p2=0.5,
  #                                  breaks = seq(0,10,2),
  #                                  limits = c(0,10)) + 
  labs(x = "Month",
       y = "Year",
       fill = "Ratio Dives\nto Dive Sites") + 
  facet_wrap(~nearest_island, scales = "free") +
  theme_bw() 
```
