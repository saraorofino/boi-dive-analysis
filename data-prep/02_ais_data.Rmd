...
To determine the length of time at a given location, we grid the AIS into 0.001 degree (or ~ 111 m) cells and sum the amount of time a vessel spends within a given grid cell.  

```{r sql-time-by-location}
sql_time_location <- "#StandardSQL
...
# Run and download 
binned_locations <- bq_project_query("emlab-gcp", sql_time_location) %>% 
  bq_table_download(n_max = Inf)
```

```{r map}
# Start by looking at tracks for operators in SB, Ventura, Oxnard
...
ggplot() + 
  geom_sf(data = ca_counties) +
  geom_sf(data = ci_polys) +
  geom_raster(data = binned_locations,
              aes(x=lon_bin,
                  y=lat_bin,
                  fill = total_hours)) + 
